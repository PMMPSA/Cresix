...
class EventListener implements Listener {
    ...
    public function openECTUI(Player $player, Item $toEnchant, Block $ectable) {
        ...
        if(empty($enchants)) {
            $player->sendMessage("§c§l[cảnh báo]§r§7 Không có bùa có sẵn cho mặt hàng này!");
            return;
        }
        /*$enchants = array(
            ...
            if($data != null) {
                switch ($data) {
                    case 1:
						...
						$player->sendMessage("§c§l[cảnh báo] bạn không đủ level để enchant!");
                            return;
                        } else {
                            $ench = Enchantment::getEnchantmentByName($arr[0]);
                            if($toEnchant->getEnchantment($ench->getId())) {
                                $player->sendMessage("§c§l[cảnh báo] bạn không dùng chug một Enchant được!");
                                return;
                            }
                    ...
                    case 2:
                        ...
                        if($player->getXpLevel() < $arr[2]) {
                            $player->sendMessage("§l§c[cảnh báo] §7bạn k đủ level để enchant");
                            return;
                        } else {
                            $ench = Enchantment::getEnchantmentByName($arr[0]);
                            if($toEnchant->getEnchantment($ench->getId())) {
                                $player->sendMessage("§l§c[cảnh báo] §abạn không thể dùng chung 1 enchant");
                                return;
                            }
                            $player->setXpLevel($player->getXpLevel() - $arr[2]);
                            $level = $arr[1];
        ...
        $form->setTitle("§a§lCresix Enchant Table§r§7 ".$toEnchant->getName());
        $form->addButton("tắt");
        ...
        $form->setContent("Enchant mục giữ của bạn");
