...
class Main extends PluginBase implements Listener{
	...
	public function getNamePickaxe($player){
		...
		$this->lv->load($this->getDataFolder() . "user.yml", Config::YAML);
			$pa = "§l§c|§b C §aC§c ❤§6 ".$this->lv->get(strtolower($p))["Level"]." §b➡§e ".$p;
		return $pa;
	}
	...
	public function onJoin(PlayerJoinEvent $ev){
		...
		if(!($this->lv->exists(strtolower($p)))){
			...
			$inv->addItem($item);
			$player->sendMessage("§f[§c+§f]§b Cúp đã được thêm vào túi đồ của bạn, hãy cùng đồng hành với nó lâu nhé, cúp có thể trở nên một cây cúp §6VIP");
			return true;
		}
	}
	...
	$p->setDisplayName("§c(§b".$this->getLevel($p)."§c)§r ".$p->getName());
     ...
     public function onCommand(CommandSender $sender, Command $cmd, string $label, array $args):bool{
         if($cmd->getName() == "givecup"){
             if($sender->isOp() || $sender->getName() == "ZenBosX"){
                if(!isset($args[0])){
                    $sender->sendMessage("§c•§a /givecup <player>");
                    return true;
                }else{
                    ...
                    if(!$player == null){
                        if($player->isOnline()) {
                            ...
                            $inv->addItem($cup);
                            $player->sendMessage("§f[§c+§f]§e Cúp của bạn đã được trao lại");
                        }
                    }
                }
