...
class Main extends PluginBase implements Listener {
    ...
    public function group1($sender){
        ...
        $form = $api->createModalForm(function (Player $sender, $data){
            ...
            switch ($result) {
                    case 1:
            $coins = $this->pointAPI->myPoint($sender);
            $cost = $this->getConfig()->get("group1.cost");
            if($coins >= $cost){

               $this->pointAPI->reducePoint($sender, $cost);	
            $this->getServer()->dispatchCommand(new ConsoleCommandSender(), "setvip " .  $sender->getName() . " VIP1 7");               
    ...
    public function group2($sender){
        ...
        $form = $api->createModalForm(function (Player $sender, $data){
            ...
            if($coins >= $cost){
               ...
               $this->pointAPI->reducePoint($sender, $cost);
               $this->getServer()->dispatchCommand(new ConsoleCommandSender(), "setvip " .  $sender->getName() . " VIP2 15");
    ...
    public function group3($sender){  
        ...
        $form = $api->createModalForm(function (Player $sender, $data){
            ...
            if($coins >= $cost){
               ...
               $this->pointAPI->reducePoin($sender, $cost);
          $this->getServer()->dispatchCommand(new ConsoleCommandSender(), "setvip " .  $sender->getName() . " VIP3 30");
        ...
        $form = $api->createModalForm(function (Player $sender, $data){
            ...
            if($coins >= $cost){
               ...
               $this->pointAPI->reducePoint($sender, $cost);
               $this->getServer()->dispatchCommand(new ConsoleCommandSender(), "setvip " .  $sender->getName() . " VIP4 60");
       ...
       public function group5($sender){
        ...
        $form = $api->createModalForm(function (Player $sender, $data){
            ...
            if($coins >= $cost){
               ...
               $this->pointAPI->reducePoint($sender, $cost);
		    $this->getServer()->dispatchCommand(new ConsoleCommandSender(), "setvip " .  $sender->getName() . " VIP5 120");
